...
def test_add_defaults():
    tree = Tree(0)
    tree.add_state(20, source=0, action=10)
...
def test_add_existing_action():
    tree = Tree(0)
    tree.add_state(1, source=0, action=10)

    with pytest.raises(ActionAlreadyExistsException):
        tree.add_state(5, source=0, action=10)
...
def test_successor_and_transitions_and_path():
    tree = Tree(0)
    tree.add_state(1, source=0, action=10)
    tree.add_state(5, source=0, action=50)
    tree.add_state(3, source=1, action=30)

    assert tree.get_actions(source=0) == {10, 50}
    assert tree.get_successor(source=0, action=10) == 1
    assert tree.get_successor(source=0, action=50) == 5
    assert tree.get_actions(source=1) == {30}
...
def test_hash_equality_is_identity():
    ...
    tree.add_state(HashableState(1), source=HashableState(0), action=1)
    tree.add_state(HashableState(2), source=HashableState(0), action=2)
    tree.add_state(HashableState(3), source=HashableState(1), action=3)
    tree.add_state(HashableState(3), source=HashableState(2), action=3)
