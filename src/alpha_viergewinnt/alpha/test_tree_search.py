...
def test_expand(empty_dummy_state_tree_search):
    ...
    assert all([tree.get_transition_attributes(root, action).action_value is None for action in actions])
    assert all([tree.get_transition_attributes(root, action).visit_count == 0 for action in actions])
    assert all([tree.get_successor(root, action).step == 1 for action in actions])
    assert all([len(tree.get_successor(root, action).played_moves) == 1 for action in actions])
    assert all([action == tree.get_successor(root, action).played_moves[0] for action in actions])
...
def test_select_leaf(empty_dummy_state_tree_search):
    ...
    assert tree.get_transition_attributes(root, selected_leaf1_action).visit_count == 1
    ...
    assert tree.get_transition_attributes(root, selected_leaf1_action).visit_count == 2
    assert tree.get_transition_attributes(selected_leaf1, selected_leaf2_action).visit_count == 1
